...
class Console extends Roact.Component<{}, {}> {
	...
	public render(): Roact.Element {
		const flareData: string[] = ["B", "Beans", "Woop Woop Fuckers"];
		const consoleMessages: [0 | 1 | 2 | 3, string, string][] = [
			[1, writeFormat(1, "CORE", "Starting in safe mode..."), "game.ReplicatedStorage.Main.Core.index:200:12"],
			[
				0,
				writeFormat(0, "TINA", "Initializing Tina!"),
				"game.ReplicatedStorage.rbxts_include.Tina.TinaCore:19:12",
			],
			[
				2,
				writeFormat(2, `CORE<font color="rgb(180,180,180)">/</font>NPC`, "'Greg' ready to interact"),
				"game.ReplicatedStorage.Main.Core.index:200:12",
			],
			[
				2,
				writeFormat(2, "CORE/NPC", "'Anabeth' ready to interact"),
				"game.ReplicatedStorage.Main.Core.index:200:12",
			],
		];
		...
		return (
			<Div
				className={`anchor-0-1 y-100% w-100% h-${
					this.portionOfScreen * 0.01 * (Workspace.CurrentCamera?.ViewportSize.Y ?? 0) > 30
						...
						if (typeIs(x, "table")) {
							return (
								<Div className="w-100% h-19px bg-transparent">
									<Span
										Text={x[1]}
										className={`w-100% h-100% text-18 text-RobotoMono text-white bg-0.8 bg-${
											FUNCTIONAL_LEVEL_COLORS[x[0]][1] ?? "transparent"
										} richtext`}
									></Span>
									<Span
										Text={x[2]}
										className={`w-100% h-100% anchor-1-0 x-100% text-18 text-Sono text-gray-300 bg-transparent richtext align-right`}
									></Span>
								</Div>
							);
						} else {
	...
	protected didMount(): void {
		this.connection = RunService.PreRender.Connect(() => {
			if (this.isResizing) {
				this.portionOfScreen =
					100 - (UserInputService.GetMouseLocation().Y / Workspace.CurrentCamera!.ViewportSize.Y) * 100;
				this.setState({});
			}
		});
	}
